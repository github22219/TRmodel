function Temp_ident = Copy_of_calculateValues(time, numtimesteps, A, E_a, Cp, M, Temp_0, T1,Tv,T2,T_260)
    ...
    for i = time
        if Tv > Temp(i) && Temp(i) >= T1
            Q_m_1(i) = A(1) * (386.7-Temp(i)) * exp(-E_a(1) ./ (8.314 * Temp(i)))*Cp*M; 
            % 计算温度变化率
            Q_m(i)=Q_m_1(i);
        ...
        elseif T2 > Temp(i) && Temp(i) >= Tv
            if Temp(i)<=386.7
            Q_m_1(i) = A(1) * (386.7-Temp(i)) * exp(-E_a(1) ./ (8.314 * Temp(i)))*Cp*M;
            Q_m_2(i) = A(2) * (480.6-Temp(i)) * exp(-E_a(2) ./ (8.314 * Temp(i)))*Cp*M;
            else
            Q_m_2(i) = A(2) * (480.6-Temp(i)) * exp(-E_a(2) ./ (8.314 * Temp(i)))*Cp*M;
            end
            % 计算温度变化率
        ...
        elseif T_260 > Temp(i) && Temp(i) >= T2
           if Temp(i)<=480.6
           Q_m_2(i) = A(2) * (480.6-Temp(i)) * exp(-E_a(2) ./ (8.314 * Temp(i)))*Cp*M;
           Q_m_3(i) = A(3) * (530.85-Temp(i)) * exp(-E_a(3) ./ (8.314 * Temp(i)))*Cp*M;
           else
           Q_m_3(i) = A(3) * (530.85-Temp(i)) * exp(-E_a(3) ./ (8.314 * Temp(i)))*Cp*M;
           end
           % 计算温度变化率
        ...
        elseif Temp(i) >=T_260 
            ...
            Q_ele(i+1) = (5000 - sum(Q_ele(1:i))) / delta_t;
            % 计算总产热功率
