...
Temp=double(Temp(19254:20359));
dTemp_dt=double(dTemp_dt(19254:20359));
...
initialGuess = double([1*10^12, 150000]); % 例如，[A_2的估计值), Ea_2的估计值]
Ub=double([1*10^15,2*10^20]);
Lb=double([1*10^5,0.2*10^2]);

% 使用 lsqcurvefit 进行参数估计
options = optimoptions('lsqcurvefit', 'Display', 'iter', 'MaxFunEvals', 10000);
